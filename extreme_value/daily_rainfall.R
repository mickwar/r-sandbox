...
calc.post = function(params){
    ...
    # Likelihood
    if (ksi != 0){
        out = k*log(zeta) - k * log(sigma) - (1 + 1/ksi)*sum(log(1+ksi*y/sigma))
    } else {
        out = k*log(zeta) - k*log(sigma) - sum(y)/sigma
        }
    # Priors
...
par(mfrow = c(3,1))
plot(density(params[,1]), main = expression(zeta), xlab = "", cex.main = 2)
