...
# X[,1] = 1
# 
# X = cbind(X, X[,2] * X[,3], ns(X[,2] / X[,3],
#     knots=seq(0.1, 0.9, by=0.2)))
# X = cbind(1, train$help.yes, train$help.total, train$help.yes*train$help.total,
#     train$year, train$year^2)
X = cbind(1, train$help.yes/train$help.total)#, ns(train$year,
    #knots = c(2002, 2007, 2013)))
X[is.na(X)] = 0
#X = cbind(X[,c(1,3)], ns(X[,2], knots = seq(0.2, 0.8, length = 3)))
X = cbind(1, ns(X[,2], df= 4))
...
#     ns(X.test[,2] / X.test[,3], knots = seq(0.1, 0.9, by = 0.2)))

X.test = cbind(1, test$help.yes/test$help.total)#, ns(test$year,
    #knots = c(2002, 2007, 2013)))
X.test[is.na(X.test)] = 0
#X.test = cbind(X.test[,c(1,3)], ns(X.test[,2],
#    knots = seq(0.2, 0.8, length = 3)))
X.test = cbind(1, ns(X.test[,2], df=4))
